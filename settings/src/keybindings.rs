...
//! write-back. Supports read, edit, add, remove, and live-apply via
//! `hyprctl reload`.
...
impl BindingsFile {
    ...
    fn parse(content: &str, path: PathBuf) -> Self {
        ...
        for raw_line in content.lines() {
            ...
            if trimmed.starts_with('#') {
                ...
                if !text.is_empty()
                    && text.chars().all(|c| c.is_uppercase() || c == ' ' || c == '(' || c == ')')
                {
        ...
        for line in &self.lines {
            match line {
                ...
                ConfigLine::Comment(s) | ConfigLine::SectionHeader(s) => {
                    // SectionHeader was already stored as a comment in original
                    // but we stored the titlecased version. Re-emit original style.
                    // Actually we stored it separately — emit as-is for comments,
                    // skip SectionHeader (they're already in Comment lines above).
                    out.push_str(s);
                    out.push('\n');
                }
    ...
    pub fn remove(&mut self, idx: usize) {
        ...
        // Remove from bindings vec and fix up indices in lines
        self.bindings.remove(idx);

        // Remove the ConfigLine::Binding(idx) entry
        self.lines.retain(|line| !matches!(line, ConfigLine::Binding(i) if *i == idx));
...
fn which_terminal() -> String {
    // Prefer $TERMINAL, then common options
...
fn parse_bind_line(line: &str, section: &str, submap: &str) -> Option<Keybinding> {
    // Format: bindXX = MODS, KEY, [DESCRIPTION,] DISPATCHER, ARGS
    ...
    if has_desc {
        // bindd = MODS, KEY, DESC, DISPATCHER, ARGS
        if parts.len() < 4 {
            return None;
        }
        Some(Keybinding {
...
fn humanize_mods(mods: &str) -> String {
    if mods.is_empty() {
        return String::new();
    }
...
/// Convert Slint key event text + modifiers to Hyprland mods + key strings.
/// Returns (mods_string, key_string) in Hyprland format.
pub fn key_event_to_hyprland(text: &str, ctrl: bool, alt: bool, shift: bool, meta: bool) -> Option<(String, String)> {
    // Build mods string
    ...
    // Map the key text to Hyprland key name
    let key = match text {
        "\n" | "\r" => "RETURN",
        " " => "SPACE",
        "\u{1b}" => "ESCAPE",
        ...
        _ if text.len() == 1 => {
            ...
            if c.is_alphanumeric() || c.is_ascii_punctuation() {
                // Return uppercase letter as the key
                return Some((mods_str, c.to_uppercase().to_string()));
            }
            return None;
        }
        // Slint special keys come as Unicode private-use chars
        _ => return None,
    };

    Some((mods_str, key.to_string()))
}
